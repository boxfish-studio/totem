...
SL_ALIGN(4)const char ReportDescriptor[34] __attribute__ ((aligned(4))) = {
		...
		0x95, USB_PACKET_SIZE,  	//Report Count (size of XMODEM Packet)
...
STATIC_CONST_STRING_DESC(iProduct, 'T', 'o', 't', 'e', 'm', ' ', 'v',
		TOTEM_VERSION_RELEASE+0x30, '.', TOTEM_VERSION_BUILD+0x30, '.',
		TOTEM_VERSION_PATCH+0x30);

STATIC_CONST_STRING_DESC(iSerialNumber, '0', '0', '0', '0', '0', '0', '0', '0',
		'1', '2', '3', '4');

const void * const USBDESC_strings[] = { &langID, &iManufacturer, &iProduct,
		&iSerialNumber };
