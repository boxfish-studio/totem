 ...
 * 						due to the CS)
 *****************************************************************************/
void spi_dma_transfer(SPI_DMA_Handler_t *spi_dma, uint8_t *txBuffer,
		uint8_t bytes) {
	...
	// Activate TX channel
	DMA_ActivateBasic(spi_dma->dma_tx_channel, true, false,
			(void *) &(spi_dma->spi_wires.usart->TXDATA), spi_dma->txBuffer,
			bytes - 1);
}
...
void spi_dma_receive(SPI_DMA_Handler_t *spi_dma, uint8_t bytes) {
	...
	// Activate RX channel
	DMA_ActivateBasic(spi_dma->dma_rx_channel, true, false, spi_dma->rxBuffer,
			(void *) &(spi_dma->spi_wires.usart->RXDATA), bytes - 1);
}
...
static void SPI_SerialPortInit(SPI_DMA_Handler_t *spi_dma) {
	...
	GPIO_PinModeSet(spi_dma->spi_wires.MOSI_Port, spi_dma->spi_wires.MOSI_Pin,
			gpioModePushPull, 0); // MOSI
	GPIO_PinModeSet(spi_dma->spi_wires.MISO_Port, spi_dma->spi_wires.MISO_Pin,
			gpioModeInput, 0); // MISO
	GPIO_PinModeSet(spi_dma->spi_wires.CLK_Port, spi_dma->spi_wires.CLK_Pin,
			gpioModePushPull, 0); // Clock
	GPIO_PinModeSet(spi_dma->spi_wires.CS_Port, spi_dma->spi_wires.CS_Pin,
			gpioModePushPull, 1); // CS
	...
	spi_dma->spi_wires.usart->ROUTE = USART_ROUTE_TXPEN | USART_ROUTE_RXPEN
			| USART_ROUTE_CSPEN | USART_ROUTE_CLKPEN |
