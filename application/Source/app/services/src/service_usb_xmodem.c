...
static void receiveCallback(uint8_t *data, uint8_t remaining);
...
void service_usb_xmodem_setup(const char * service_name,
	...
	xTaskCreate(service_usb_xmodem, service_name, 750, NULL, service_priority,
			&handle_xmodem);
}
	...
	while (1) {
		...
		// Sending data to PC
			...
			case 2: {
				...
				if (timeout_counter > XMODEM_TIMEOUT) {
					PRINT ("task_xmodem_communicator(): Timeout occured in sending switch (state 2)! Reset task to initial state... \n");
					current_task_mode = IDLE;
					send_state = 0;
					timeout_counter = 0;
					continue;
				...
				if (xmodem_send(send_data, first_send_run,
						usb_in_data.data[0] == XMODEM_ACK))
					send_state++;
				else {
			...
			case 4: {
				...
				if (usb_in_data.data[0] != XMODEM_ACKEOT) {
					PRINT ("task_xmodem_communicator(): send_state 4: Didn't receive ACKEOT\n");
					//TODO: Make timer local
					timeout_counter++;

					if (timeout_counter > XMODEM_TIMEOUT) {
						PRINT ("task_xmodem_communicator(): send_state 4: Timeout happened while waiting for ACKEOT! Reset task to initial state... \n");
						current_task_mode = IDLE;
						send_state = 0;
						timeout_counter = 0;
						continue;
...
static bool usb_send(uint8_t data[], int len) {
	...
	if (!USBD_EpIsBusy(HID_INTR_IN_EP_ADDR)) {
...
static int xmodem_verifyPacketChecksum(xModem_packet_t *pkt,
	...
	calculatedCRC = crc_calc((uint8_t *) pkt->data,
			(uint8_t *) &(pkt->crcHigh));
